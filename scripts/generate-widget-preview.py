...
def draw_preview(project: Path, dark: bool):
    ...
    switch_background = '#39433D' if dark else '#E9DDD1'
    switch_text = '#F2F4F1' if dark else '#493F37'
    image = Image.new('RGBA', (840, 360), (0, 0, 0, 0))
    draw = ImageDraw.Draw(image)
    draw.rounded_rectangle((0, 0, 839, 359), radius=60, fill=background)
    draw.text((42, 34), '瓶喂奶量 · 近7天', font=ImageFont.truetype(FONT, 42), fill=title)
    draw.rounded_rectangle((640, 24, 798, 84), radius=24, fill=switch_background)
    draw.text((719, 54), '切换图表', font=ImageFont.truetype(FONT, 34), fill=switch_text, anchor='mm')
    ...
    day_font = ImageFont.truetype(FONT, 31)
    ...
    draw.text((42, 315), '日均 180ml（不包含今日）', font=ImageFont.truetype(FONT, 36), fill=muted)
