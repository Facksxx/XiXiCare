...
import { SOUND_PACKS, type SoundIconName } from '../utils/soundPacks';
...
export function WhiteNoisePlayer({ isOpen, onClose, onPlaybackChange }: WhiteNoisePlayerProps) {
  ...
  useEffect(() => {
    setPackTracks(SOUND_PACKS.flatMap(pack => pack.tracks.map(track => ({
      id: track.id,
      name: track.name,
      src: `/sound-packs/${pack.id}/${track.file}`,
      nativeSrc: `file:///android_asset/public/sound-packs/${pack.id}/${track.file}`,
